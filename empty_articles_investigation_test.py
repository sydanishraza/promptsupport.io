...
def find_test_files():
    ...
    for filename in potential_files:
        ...
        if os.path.exists(file_path):
            ...
            log_investigation(f"✅ Found test file: {filename} ({file_size:,} bytes, {file_size/1024/1024:.1f}MB)")
    ...
    for file_path in docx_files + pdf_files:
        ...
        if not any(f['name'] == filename for f in test_files):
            ...
            log_investigation(f"✅ Found additional file: {filename} ({file_size:,} bytes, {file_size/1024/1024:.1f}MB)")
    
    log_investigation(f"📁 Total files found for testing: {len(test_files)}")
    return test_files
...
def get_content_library_baseline():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_investigation(f"📚 Content Library baseline: {total_articles} articles")
            return total_articles, data.get('articles', [])
        else:
            log_investigation(f"❌ Failed to get Content Library baseline: {response.status_code}")
            return 0, []
    except Exception as e:
        log_investigation(f"❌ Error getting Content Library baseline: {e}")
        return 0, []
...
def analyze_article_content(articles, source_filename):
    ...
    log_investigation(f"🔍 ANALYZING {len(articles)} ARTICLES from {source_filename}")
    ...
    for i, article in enumerate(articles):
        ...
        if is_empty:
            ...
            log_investigation(f"❌ EMPTY ARTICLE FOUND: '{title}' (ID: {article_id})")
        elif has_html_placeholders:
            analysis['html_placeholder_articles'] += 1
            log_investigation(f"⚠️ HTML PLACEHOLDER ISSUE: '{title}' (ID: {article_id})")
        elif is_very_short:
            analysis['very_short_articles'] += 1
            log_investigation(f"⚠️ VERY SHORT ARTICLE: '{title}' ({content_length} chars)")
        else:
            analysis['populated_articles'] += 1
            log_investigation(f"✅ POPULATED ARTICLE: '{title}' ({content_length} chars)")
...
def process_file_and_investigate(file_info):
    ...
    log_investigation(f"🎯 PROCESSING FILE: {filename} ({file_size_mb:.1f}MB)", "CRITICAL")
    ...
    try:
        ...
        log_investigation(f"📤 Uploading {filename}...")
        ...
        with open(file_path, 'rb') as f:
            ...
            if response.status_code != 200:
                ...
                log_investigation(f"❌ {error_msg}", "ERROR")
            ...
            log_investigation(f"✅ Upload successful, Job ID: {job_id}")
        
        # Monitor processing
        log_investigation("⏳ Monitoring processing...")
        ...
        while True:
            ...
            try:
                ...
                if status_response.status_code == 200:
                    ...
                    log_investigation(f"📊 Status: {status} (elapsed: {elapsed:.1f}s)")
                    ...
                    if status == 'completed':
                        ...
                        log_investigation(f"✅ Processing completed in {processing_time:.1f} seconds")
                        log_investigation(f"📈 Chunks Created: {chunks_created}")
                        log_investigation(f"📄 Articles Generated: {articles_generated}")
                    ...
                    log_investigation(f"⚠️ Status check failed: {status_response.status_code}")
                    time.sleep(5)
                    
            except Exception as e:
                log_investigation(f"⚠️ Status check error: {e}")
                time.sleep(5)
        
        # Get updated Content Library and analyze new articles
        log_investigation("🔍 Analyzing generated articles...")
        ...
        log_investigation(f"📚 Article count change: {baseline_count} → {final_count} (+{new_articles_count})")
        ...
        log_investigation(f"🔍 Identified {len(new_articles)} new articles for analysis")
        ...
        if new_articles:
            ...
            if article_analysis['empty_articles'] > 0:
                log_investigation(f"🚨 CRITICAL ISSUE: {article_analysis['empty_articles']} EMPTY ARTICLES FOUND", "CRITICAL")
            
            if article_analysis['html_placeholder_articles'] > 0:
                log_investigation(f"⚠️ HTML PLACEHOLDER ISSUE: {article_analysis['html_placeholder_articles']} articles with HTML placeholders", "WARNING")
            
            if article_analysis['populated_articles'] > 0:
                log_investigation(f"✅ SUCCESS: {article_analysis['populated_articles']} articles with proper content", "SUCCESS")
...
def run_comprehensive_empty_articles_investigation():
    ...
    log_investigation("🚨 STARTING CRITICAL EMPTY ARTICLES BUG INVESTIGATION", "CRITICAL")
    ...
    if not test_files:
        log_investigation("❌ No test files found for investigation", "CRITICAL")
        return
    
    # Test backend health first
    log_investigation("🏥 Testing backend health...")
    try:
        response = requests.get(f"{API_BASE}/health", timeout=30)
        if response.status_code != 200:
            log_investigation(f"❌ Backend health check failed: {response.status_code}", "CRITICAL")
            return
        log_investigation("✅ Backend health check passed")
    except Exception as e:
        log_investigation(f"❌ Backend health check failed: {e}", "CRITICAL")
        return
    ...
    for i, file_info in enumerate(test_files, 1):
        ...
        if i < len(test_files):
            log_investigation("⏳ Waiting 30 seconds before next test...")
            time.sleep(30)
    
    # Generate comprehensive analysis report
    log_investigation(f"\n{'='*80}")
    log_investigation("🎯 COMPREHENSIVE INVESTIGATION RESULTS", "CRITICAL")
    ...
    log_investigation(f"📊 OVERALL STATISTICS:")
    ...
    for result in investigation_results:
        ...
        log_investigation(f"   Processing: {'✅ Success' if result['processing_successful'] else '❌ Failed'}")
        ...
        if result['article_analysis']:
            ...
            log_investigation(f"   📋 ARTICLE ANALYSIS:")
            ...
            for article in analysis['content_analysis']:
                if article['is_empty']:
                    log_investigation(f"      🚨 EMPTY: '{article['title']}' (ID: {article['id']})")
                elif article['has_html_placeholders']:
                    log_investigation(f"      ⚠️ HTML ISSUE: '{article['title']}' (ID: {article['id']})")
        
        if result['errors']:
            log_investigation(f"   ❌ ERRORS:")
            for error in result['errors']:
                log_investigation(f"      {error}")
    
    # Final summary
    log_investigation(f"\n🎯 CRITICAL FINDINGS SUMMARY:")
    log_investigation(f"   🚨 Total Empty Articles Found: {empty_articles_found}")
    log_investigation(f"   ⚠️ HTML Placeholder Issues: {html_placeholder_issues}")
    log_investigation(f"   ✅ Properly Populated Articles: {populated_articles}")
    
    # Determine root cause patterns
    log_investigation(f"\n🔍 ROOT CAUSE ANALYSIS:")
    
    if empty_articles_found > 0:
        log_investigation(f"🚨 CRITICAL BUG CONFIRMED: {empty_articles_found} empty articles generated", "CRITICAL")
        ...
        if unified_empty > 0 and split_empty > 0:
            log_investigation("   🔍 PATTERN: Both unified and split processing affected")
        elif unified_empty > 0:
            log_investigation("   🔍 PATTERN: Only unified processing affected")
        elif split_empty > 0:
            log_investigation("   🔍 PATTERN: Only split processing affected")
        ...
        log_investigation("✅ NO EMPTY ARTICLES FOUND - Bug may be resolved or not reproducible with current files")
    
    if html_placeholder_issues > 0:
        log_investigation(f"⚠️ HTML CLEANING ISSUES: {html_placeholder_issues} articles with HTML placeholder problems")
...
if __name__ == "__main__":
    ...
    if results:
        ...
        if empty_articles_found > 0:
            print(f"\n❌ CRITICAL BUG CONFIRMED: {empty_articles_found} empty articles found")
            sys.exit(1)  # Critical bug found
        else:
            print(f"\n✅ NO EMPTY ARTICLES FOUND: Bug not reproduced with current test files")
            sys.exit(0)  # No critical issues
    else:
        print(f"\n❌ INVESTIGATION FAILED: Could not complete testing")
