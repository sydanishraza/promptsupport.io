...
const EnhancedAssetManager = ({ 
  ...
  useEffect(() => {
    ...
    fetchAllAssets();
  }, [articles, backendUrl, deletedAssetIds, refreshCounter]);
  ...
  return (
    <div className="h-full flex flex-col space-y-4">
      ...
      {/* Upload Modal */}
      {console.log('🎭 Rendering upload modal section, showUploadModal:', showUploadModal)}
      <AnimatePresence>
        {showUploadModal && (
          <>
            {console.log('🎭 Upload modal should be visible!')}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
