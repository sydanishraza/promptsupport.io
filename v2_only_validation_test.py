...
""",
    ...
    def test_pipeline_orchestration_v2_only(self):
        ...
        try:
            # Test V2 pipeline through content processing endpoint
            test_payload = {
                "content": "# Pipeline Orchestration Test\n\nTesting V2-only pipeline orchestration with comprehensive content processing.",
                "content_type": "text"
            }
            ...
            if response.status_code != 200:
                self.log_test("Pipeline Orchestration V2-Only", False, 
                             f"V2 pipeline processing failed: HTTP {response.status_code}")
                return False
            ...
            if data.get("status") not in ["completed", "success"]:
                self.log_test("Pipeline Orchestration V2-Only", False, 
                             f"V2 pipeline processing unsuccessful: {data.get('status')}")
                return False
            ...
            # Check V2-only mode flag
            v2_only_mode = data.get("v2_only_mode", False)
            if not v2_only_mode:
                self.log_test("Pipeline Orchestration V2-Only", False, 
                             "V2-only mode not confirmed in processing response")
                return False
            ...
            v2_indicators = [
                "v2" in str(metadata).lower(),
                article.get("id") is not None,  # Repository pattern used
                len(article.get("content", "")) > 50  # Content was processed
            ]
            
            v2_processing_score = sum(v2_indicators) / len(v2_indicators) * 100
            
            if v2_processing_score < 66:  # At least 2/3 indicators should be present
                self.log_test("Pipeline Orchestration V2-Only", False, 
                             f"Low V2 processing indicators: {v2_processing_score:.1f}%")
                return False
            
            self.log_test("Pipeline Orchestration V2-Only", True, 
                         f"V2 pipeline orchestration verified: engine=v2, v2_only_mode=true, {len(articles)} articles generated")
            return True
